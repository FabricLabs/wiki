...
REPO_STORE=./stores/snippets
ASSETS_OUT=./assets/snippets
TARGET_BRANCH=master
COMMAND_TO_RUN="mkdir -p $ASSETS_OUT && cp $REPO_STORE/snippets/* $ASSETS_OUT"
...
case $CHOICE in
  [yY][eE][sS]|[yY])
    ...
    eval $COMMAND_TO_RUN
    echo "All done!  The latest snippets should now be located in ./assets/snippets"
    ;;
  [nN][oO]|[nN])
